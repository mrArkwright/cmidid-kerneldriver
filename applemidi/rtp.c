 ...
 * @param ssrc The synchronization source identifier that uniquely identifies
 * the peer.
 * @param size The size of the address pointed to by @c addr.
 * @param addr A pointer to an address that can be used to send packets to the
 * client.
 * @return a pointer to the created peer structure on success.
 * @return a @c NULL pointer if the peer could not created.
 */
struct RTPPeer *RTPPeerCreate(unsigned long ssrc, int size,
			      struct sockaddr_in *addr)
{
	...
	pr_info("added RTP client %pI4:%d\n",
		&peer->address.addr.sin_addr.s_addr,
		peer->address.addr.sin_port);
	peer->in_seqnum = 0;
	peer->in_timestamp = 0;
	peer->out_seqnum = 0;
 ...
 */
int RTPPeerGetAddress(struct RTPPeer *peer, int *size,
		      struct sockaddr_in **addr)
{
 ...
 * Lookup the peer using the (pseudo) hash-table, add it to the list and retain
 * it.
 ...
 * Lookup the peer using the (pseudo) hash-table, remove it from the list and
 * release it.
 * @public @memberof RTPSession
 * @param session The session.
...
static int _rtp_encode_header(struct RTPPacketInfo *info, size_t size,
			      void *data, size_t *written)
{
	int i, j;
	...
	buffer[0] = 0x80 | (info->padding ? 0x20 : 0) |
		    (info->extension ? 0x10 : 0) | (info->csrc_count & 0x0f);

	buffer[1] = (info->payload_type & 0x7f) | (info->marker ? 0x80 : 0x00);
...
static int _rtp_encode_extension(struct RTPPacketInfo *info, size_t size,
				 void *data, size_t *written)
{
	int i;
	unsigned char *buffer = data;
...
static int _rtp_encode_padding(struct RTPPacketInfo *info, size_t size,
			       void *data, size_t *written)
{
	unsigned char *buffer = data;
...
static void _append_iov(size_t *iovlen, struct iovec *iov, size_t size,
			void *buffer)
{
...
int RTPSessionSendPacket(struct RTPSession *session, struct RTPPacketInfo *info)
{
	...
	while ((iovlen - 1) < info->iovlen) {
		...
		_append_iov(&iovlen, &(iov[0]), info->iov[iovlen - 1].iov_len,
			    info->iov[iovlen - 1].iov_base);
	}
	...
	for (i = 0; i < iovlen; i++) {
		pr_debug("[%i] iov_len: %i, iov_base: %p\n", i,
			 (int)iov[i].iov_len, iov[i].iov_base);
		for (j = 0; j < iov[i].iov_len; j++) {
			unsigned char c =
			    *((unsigned char *)iov[i].iov_base + j);
	...
	pr_debug("send %i bytes to %pI4:%i on s(%p)\n", info->total_size,
		 &a->sin_addr.s_addr, ntohs(a->sin_port), session->socket->sk);
