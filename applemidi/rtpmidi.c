...
static int _rtpmidi_encode_header(struct RTPMIDIInfo *info, size_t size,
				  void *data, size_t *written)
{
	...
	if (info->len > 0x0f) {
		if (size < 2)
			return 1;
		buffer[0] = 0x80 | (info->journal ? 0x40 : 0) |
			    (info->zero ? 0x20 : 0) |
			    (info->phantom ? 0x10 : 0) |
			    ((info->len >> 8) & 0x0f);
		...
		*written = 2;
		...
		buffer[0] = (info->journal ? 0x40 : 0) |
			    (info->zero ? 0x20 : 0) |
			    (info->phantom ? 0x10 : 0) | (info->len & 0x0f);
		*written = 1;
	}
	return 0;
}

static int _rtpmidi_encode_messages(struct RTPMIDIInfo *info,
				    MIDITimestamp timestamp,
				    struct MIDIMessageList *messages,
				    size_t size, void *data, size_t *written)
{
	int m, result = 0;
	void *buffer = data;
	size_t w;
	...
	for (m = 0;
		...
		MIDIMessageEncodeRunningStatus(messages->message, &status, size,
					       buffer, &w);
 ...
 * Broadcast the messages to all connected peers. Store the number of sent
 * messages
 * in @c count, if the @c info argument was specified it will be populated with
 * the
		       ...
		       struct MIDIMessageList *messages)
{
	...
	_rtpmidi_encode_messages(minfo, timestamp, messages, size, buffer,
				 &written);
	...
	while (peer != NULL) {
		...
		//  _rtpmidi_journal_encode( session, journal, size, buffer,
		// &written );
		...
		if (result == 0 && minfo->journal) {
			//_rtpmidi_journal_encode_messages( journal,
			// info->sequence_number, messages );
		}
