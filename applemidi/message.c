...
static int _check_encode_running_status(unsigned char byte,
...
static int _check_decode_running_status(unsigned char byte,
					MIDIRunningStatus *status)
{
	if (status == NULL || *status == 0)
		return 0;
...
static int _update_running_status(struct MIDIMessageData *data,
				  MIDIRunningStatus *status)
{
	if (status == NULL)
		return 0;
...
static int _encode_three_bytes(struct MIDIMessageData *data,
			       MIDIRunningStatus *status, size_t size,
			       void *buffer, size_t *written)
{
...
static int _decode_three_bytes(struct MIDIMessageData *data,
			       MIDIRunningStatus *status, size_t size,
...
#define PROPERTY_CASE_SET(flag, type, field)                                   \
	...
	if ((*((type *)value) & ((flag == MIDI_STATUS) ? 0xff : 0x7f)) !=      \
	    *((type *)value))                                                  \
		return 1;                                                      \
	field = *((type *)value);                                              \
	return 0
...
#define PROPERTY_CASE_SET_H(flag, type, field)                                 \
	PROPERTY_CASE_BASE(flag, type);                                        \
	if ((*((type *)value) & ((flag == MIDI_STATUS) ? 0x0f : 0x07)) !=      \
	    *((type *)value))                                                  \
		return 1;                                                      \
...
#define PROPERTY_CASE_GET_H(flag, type, field)                                 \
	PROPERTY_CASE_GET(flag, type, MIDI_HIGH_NIBBLE(field))
#define PROPERTY_CASE_GET_L(flag, type, field)                                 \
	PROPERTY_CASE_GET(flag, type, MIDI_LOW_NIBBLE(field))
 ...
 */
static int _set_note_off_on(struct MIDIMessageData *data, MIDIProperty property,
 ...
 */
static int _get_note_off_on(struct MIDIMessageData *data, MIDIProperty property,
			    size_t size, void *value)
{
	unsigned char *m = &(data->bytes[0]);
	if (size == 0 || value == NULL)
...
static struct MIDIMessageFormat _note_off_on = {
	&_test_note_off_on, &_size_three_bytes,   &_set_note_off_on,
	&_get_note_off_on,  &_encode_three_bytes, &_decode_three_bytes
};
 ...
 * @return a pointer to the correct message format if the format could be
 * detected.
 ...
 */
int MIDIMessageFormatEncodeRunningStatus(struct MIDIMessageFormat *format,
					 ...
					 void *buffer, size_t *written)
{
	...
	if (format != NULL) {
		...
		if (format->encode != NULL) {
			...
			return (format->encode)(data, status, size, buffer,
						written);
		} else {
 ...
 * Encode message objects into a buffer. Use the running status and update it if
 * necessary.
 * @public @memberof MIDIMessageFormat
 * @param message  The message.
				   ...
				   unsigned char *buffer, size_t *written)
{
	// MIDIPrecond( message != NULL, EFAULT );
	// MIDIPrecond( size > 0 && buffer != NULL , EINVAL);
	return MIDIMessageFormatEncodeRunningStatus(
	    message->format, &(message->data), status, size, buffer, written);
}