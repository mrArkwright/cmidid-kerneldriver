...
Alsa sequencer -> Applemidi -> Network -> any compatible client

The Protocol is based on RTPMIDI specified in (RFC 4695, obsoleted) (http://tools.ietf.org/html/rfc6295 "RFC 6295") with a proprietary extension from Apple.
...
If you want to use this module on an architecture where the `__aeabi_uldivmod` symbol is available you have to remove the `_uldivmod.o` target from `applertp-objs`
...
Remember: The control port and the next higher have to be free, otherwise the module will fail to listen on these ports.
...
On any supporting other device you can now connect to this service by entering its hostname or IP and the port you have assigned.
